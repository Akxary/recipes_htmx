...
from resources.config import SEND_CODE_LIMIT, SESSION_TIME_LIMIT, VERIFICATION_CODE_LIMIT
...
class UserManager:
    ...
    async def check_verification_code(self, verification_code: int) -> VerificationResult:
        user = await self.get_user()
    ...
    async def activate_user_session(self)->User:
        query = update(User).where(User.user_email==self.user_email).values({"active_time": get_current_time()}).returning(User)
    ...
    async def check_user_session(self)->bool:
        ...
        if current_time > user_active_time:
            logger.warning("User %s session expired at %s", self.user_email, user_active_time)
            return False
        return True
